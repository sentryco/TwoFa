...
/**
 * OneTimePassword - OTP
 * - Description: The OTP struct represents a One-Time Password, a password
 *                that is valid for only one login session or transaction. It
 *                is commonly used in two-factor authentication systems to
 *                provide an additional layer of security beyond just a
 *                username and password. The OTP struct contains the secret
 *                key, the period for which the OTP is valid, the number of
 *                digits in the OTP, and the hashing algorithm used to
 *                generate the OTP.
 * - Note: A one-time password generator is a security tool used to generate
 *         a unique password that can only be used once. The purpose of a
 *         one-time password generator is to provide an additional layer of
 *         security to user accounts by requiring a new password for each
 *         login attempt. This helps to prevent unauthorized access to user
 *         accounts, even if the password is compromised. One-time password
 *         generators are commonly used in two-factor authentication (2FA)
 *         systems, where the user is required to provide both a password and
 *         a one-time password to access their account. This helps to ensure
 *         that only authorized users are able to access sensitive
 *         information or perform critical actions.
 * - Remark: `GeneratorType` is in `OTPAccount`
 */
public struct OTP {
   /**
    * - Abstract: This needs to be data, because there are many types to embed
    *             the data, base32, base64 etc etc ASCII etc (Secret key data)
   ...
   /**
    * Digits "must" be between 6 and 8 inclusive (Number of digits for
    * generated string in range 6...8, defaults to 6)
    * - Description: The digits property specifies the length of the OTP. It
    *                must be within the range of 6 to 8 characters to conform
    *                to common standards and ensure a balance between
    ...
    * - Description: The 'algo' property represents the cryptographic hash
    *                function used to generate the OTP. It determines how
    *                the secret key is transformed into a one-time password.
    *                Supported algorithms include SHA-1, SHA-256, and SHA-512,
    *                with SHA-1 being the least secure and SHA-512 being the
    *                most secure.
    ...
    * - Description: Initializes a counter-based one-time password (OTP)
    *                object with a secret key, period of validity, number of
    *                digits, and a hashing algorithm. This OTP object can be
    *                used to generate passwords that are valid for a single
    *                use within the specified time frame, providing an
