 ...
 * - Description: This class is responsible for managing the countdown timer for the OTP (One-Time Password) process. It provides functionality to start the timer, update it every second, and perform a callback function on each timer tick. This is crucial for OTPs, especially TOTP (Time-Based One-Time Password), where the OTP is valid only for a certain period of time (e.g., 30 or 60 seconds).
 */
public final class OTPTimer {
   /**
    * Ticker instance
    * - Description: This is the timer instance that is used to manage the countdown functionality in the OTP process.
    */
   fileprivate var timer: Timer?
   /**
    * Callback for timer ticks
    * - Description: The `onChange` closure is executed every time the timer ticks. It allows the OTPTimer to perform any necessary updates each second, such as refreshing the UI to show the remaining time for the OTP or updating the OTP value itself.
    */
   fileprivate var onChange: OTPTimerCallback = defaultOnChange
   /**
    * - Description: The `onChange` closure is executed every time the timer ticks. It allows for custom actions to be performed at each tick, such as updating a user interface or processing the remaining time.
    * ## Examples:
...
extension OTPTimer {
    ...
    * - Description: This method starts the countdown timer for the OTP process. It creates a new timer that fires every second and adds it to the main run loop. The timer triggers the `update` method at each interval, which in turn triggers the `onChange` closure. This allows for custom actions to be performed at each tick, such as updating a user interface or processing the remaining time.
   ...
   /**
    * Stop the timer
    * - Description: This method stops the timer, effectively halting the countdown process. It is typically called when the OTP is no longer needed or when a new OTP is about to be generated.
   ...
   /**
    * Update
    * - Description: This method is called by the timer at each interval defined in the `start` method. It triggers the `onChange` closure, allowing the timer to perform any actions that have been set when the timer was initialized or modified.
    */
...
extension OTPTimer {
    ...
    * - Description: This method calculates the progress of the current OTP period as a percentage. It determines how much time has elapsed in the current OTP period relative to the total period duration, providing a visual representation of the time remaining before the OTP expires.
    * - Parameters:
    ...
    * - Description: Calculates the remaining time interval before the current period of the OTP expires. This is used to determine how much time is left before the OTP needs to be refreshed.
    * - Parameters:
    *   - date: The time to start from
    *   - period: The amount of time (30 / 60sec) etc
    */
...
extension OTPTimer {
    ...
    * - Description: Defines the signature for the callback function that is invoked when the timer updates. This callback is used to perform actions in response to each tick of the timer.
    * - Parameter otpTimer: The `OTPTimer` object that is calling the callback.
    */
   public typealias OTPTimerCallback = (_ otpTimer: OTPTimer) -> Void
   /**
    * This is the default callback function for the OTP timer. 
    * - Description: If no custom callback function is provided when the OTP timer is initialized or modified, this default function will be called instead. It simply prints "Default onChange" to the console.
